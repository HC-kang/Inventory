...
@router.get("/{storage_id}", status_code=200, response_model=Storage)
...
async def search_storage(
    *,
    keyword: str = Query(None, min_length=2, example="창고"),
...
async def create_storage(
    *,
    ...
    if storage_in.user_id != current_user.id:
        raise HTTPException(
            status_code=403, detail=f"오직 자신의 저장소만 등록 할 수 있습니다."
        )
...
async def update_storage(
    *,
    ...
    if not storage:
        raise HTTPException(
            status_code=404, detail=f"{storage_in.id}번 저장소를 찾을 수 없습니다."
        )
    if storage.user_id != current_user.id:
        raise HTTPException(
            status_code=403, detail=f"자신의 저장소 정보만 수정 할 수 있습니다."
        )
